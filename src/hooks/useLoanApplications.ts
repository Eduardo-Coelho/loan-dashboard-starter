...
import { ApplicationFilters, ApplicationStatus, LoanApplication } from '../types'
...
export type SortField = 'applicantName' | 'amount' | 'riskScore' | 'submittedAt'
...
function sortApplications(apps: LoanApplication[], sort: SortConfig): LoanApplication[] {
...
async function simulateStatusMutation(_id: string, _status: ApplicationStatus): Promise<void> {
  return new Promise<void>((resolve, reject) =>
    setTimeout(() => {
      if (Math.random() < 0.1) {
        reject(new Error('Server error: status update failed. Changes rolled back.'))
      } else {
        resolve()
      }
    }, 800),
...
export function useLoanApplications(initialFilters?: ApplicationFilters) {
  ...
  const applications = useMemo(() => {
    ...
    if (filters.searchTerm) {
      ...
      data = data.filter((app) => app.applicantName.toLowerCase().includes(term))
    }
  ...
  const updateStatus = useCallback(
    async (id: string, newStatus: ApplicationStatus): Promise<void> => {
      const previousStatus =
        statusOverrides[id]?.status ?? mockApplications.find((a) => a.id === id)?.status

      setMutationError(null)

      // Optimistic update — immediately reflect the change in the UI
      setStatusOverrides((prev) => ({ ...prev, [id]: { status: newStatus, isPending: true } }))

      try {
        await simulateStatusMutation(id, newStatus)
        // Commit: mark as no longer pending
        setStatusOverrides((prev) => ({ ...prev, [id]: { status: newStatus, isPending: false } }))
      } catch (err) {
